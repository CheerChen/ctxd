...
### Fixed
* **Slack downloads were invisible in the export** : `--download-files` wrote binaries to `attachments/IMG_<file_id>.<ext>` but every attachment line still showed only the Slack permalink, so nothing in the artifact revealed that a local copy existed or where it was. Downloaded files now append `— saved: <relative path>` to their attachment line, keeping the permalink for humans. Files that failed to download get no path, so the line never points at something absent.
* **Slack said nothing about files it skipped** : a thread with attachments and no `--download-files` produced a summary mentioning only message and participant counts. The summary now reports either how many files were downloaded and where, or how many were left behind and which flag to use — once per thread rather than once per message.
* **Confluence images rendered as dead links unless `-i` was used** : embedded images fell back to `images/<filename>`, a relative path that only resolves inside a directory export with `-i`. Every other mode — stdout, single-file, Obsidian, directory export without `-i` — emitted a link to a directory that was never created, and unlike Jira there was no remote URL left to fall back on, so the image was unreachable from the artifact. Images not saved locally now point at the v1 REST attachment download endpoint, which accepts API-token Basic auth, 302-redirects to a freshly signed media URL per request, and therefore embeds no token and does not expire. Resolution order is local copy → download URL → the old relative path (still used for drawio/plantuml output that has no matching attachment). Costs one `get_attachments` call, made only for pages that actually embed images, and reused from the existing `-i` fetch so an `-i` run makes no extra call. Pages whose images were not downloaded say so in the run summary, naming any image with no attachment URL at all.
