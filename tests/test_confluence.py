...
def test_get_space_name_resolves_and_caches() -> None:
    ...
    mock_response.json.return_value = {"id": "98765", "name": "Test Space"}
    mock_response.raise_for_status.return_value = None
    with patch.object(client.session, "get", return_value=mock_response) as mock_get:
        assert client.get_space_name("98765") == "Test Space"
...
def _make_page(
    *,
    ...
    webui: str = "/spaces/TEST/pages/123/Example",
...
def _make_dumper():
    ...
    dumper = ConfluenceDumper(
        url="https://example.atlassian.net/wiki/spaces/TEST/pages/123/Example",
        output=None,
        fmt="md",
        quiet=True,
    )
    ...
    client._user_cache["acc-author"] = "Test Author"
...
def test_build_metadata_block_happy_path() -> None:
    ...
    assert "| **Space** | Test Space |" in block
    assert "| **Author** | Test Author |" in block
...
def test_transform_prepends_metadata_block_and_offsets_marker_map() -> None:
    ...
    assert "| **Space** | Test Space |" in result
