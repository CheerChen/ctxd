...
def _make_confluence_dumper(tmp_path: Path, *, output: str | None, auto: bool):
    ...
    dumper = ConfluenceDumper(
        url="https://kinto-dev.atlassian.net/wiki/spaces/DBRE/pages/12345/Design",
        output=output,
        fmt="md",
        quiet=True,
        obsidian_mode=True,
...
def _stub_page(page_id: str = "12345", title: str = "Design Doc", body_html: str = "<p>hello</p>") -> dict:
    return {
        ...
        "_links": {"webui": f"/spaces/DBRE/pages/{page_id}/{title}"},
...
def test_confluence_obsidian_writes_frontmatter_and_body(tmp_path: Path) -> None:
    ...
    assert "confluence_url: https://kinto-dev.atlassian.net/wiki/spaces/DBRE/pages/12345/Design\n" in content
...
def _make_jira_dumper(*, output: str | None, auto: bool):
    ...
    dumper = JiraDumper(
        url="https://kinto-dev.atlassian.net/browse/PROJ-1",
        output=output,
        fmt="md",
        quiet=True,
        obsidian_mode=True,
...
def test_jira_obsidian_writes_frontmatter_and_body(tmp_path: Path) -> None:
    ...
    content = output_file.read_text(encoding="utf-8")
    assert content.startswith("---\n")
    assert "jira_url: https://kinto-dev.atlassian.net/browse/PROJ-1\n" in content
